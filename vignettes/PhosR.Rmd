...
ppe@Residue <- gsub("[0-9]","", sapply(strsplit(rownames(ppe), ";"), "[[", 2))
ppe@Site <- as.numeric(gsub("[A-Z]","", sapply(strsplit(rownames(ppe), ";"), "[[", 2)))
ppe@Sequence <- sapply(strsplit(rownames(ppe), ";"), "[[", 4)
```
Collectively, we can set up our data as following.
```{r}
ppe <- PhosphoExperiment(assays = list(Quantification = as.matrix(phospho.L6.ratio)), 
                  Site = as.numeric(gsub("[A-Z]","", sapply(strsplit(rownames(ppe), ";"), "[[", 2))),
                  GeneSymbol = sapply(strsplit(rownames(ppe), ";"), "[[", 2),
                  Residue = gsub("[0-9]","", sapply(strsplit(rownames(ppe), ";"), "[[", 2)),
...
```{r}
ppe_imputed_scaled <- medianScaling(ppe_imputed, scale = FALSE, imputed = TRUE)
```
...
p2 = plotQC(ppe_imputed_scaled@assays@data$scaled, 
       labels=colnames(ppe_imputed_scaled), panel = "quantify", cols = cols)
