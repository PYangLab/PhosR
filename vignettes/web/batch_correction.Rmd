...
```{r}
<<<<<<< HEAD:vignettes/batch_correction.Rmd
cs = rainbow(length(unique(grps)))
colorCodes = sapply(grps, switch, AICAR=cs[1], Ins=cs[2], AICARIns=cs[3])

par(mfrow=c(1,1))
plotQC(ppe@assays@data$Quantification, 
       panel = "dendrogram", 
       cols = colorCodes)
=======
plotQC(ppe@assays@data$Quantification, panel = "dendrogram", cols=grps, labels = colnames(ppe)) +
  ggplot2::ggtitle("Before batch correction")
>>>>>>> 6e3f8e1c8392e2dc1f724b23c5617fbcb209cf64:vignettes/web/batch_correction.Rmd
```
...
plotQC(ppe@assays@data$Quantification, 
       panel = "pca", 
       labels = colnames(ppe), 
       ylim = c(-20, 20), xlim = c(-30, 30), 
       cols = colorCodes)
=======
plotQC(ppe@assays@data$Quantification, cols=grps, labels = colnames(ppe), panel = "pca") +
  ggplot2::ggtitle("Before batch correction")
>>>>>>> 6e3f8e1c8392e2dc1f724b23c5617fbcb209cf64:vignettes/web/batch_correction.Rmd
```
...
par(mfrow=c(1,2))
plotQC(ppe@assays@data$Quantification, panel = "dendrogram", cols=colorCodes, labels = colnames(ppe))
plotQC(ppe@assays@data$Quantification, cols=colorCodes, labels = colnames(ppe), panel="pca", ylim=c(-20, 20), xlim=c(-30, 30))
```

```{r}
# plot after batch correction
par(mfrow=c(1,2))
plotQC(ppe@assays@data$normalised, panel = "dendrogram", cols=colorCodes, labels = colnames(ppe))
plotQC(ppe@assays@data$normalised, cols=colorCodes, labels = colnames(ppe_RUV), panel="pca", ylim=c(-20, 20), xlim=c(-30, 30))
=======
# plot after batch correction
p1 = plotQC(ppe@assays@data$Quantification, cols=grps, labels = colnames(ppe), panel = "dendrogram" )
p2 = plotQC(ppe@assays@data$normalised, cols=grps, labels = colnames(ppe), panel="dendrogram")
ggpubr::ggarrange(p1, p2, nrow = 1)
```

```{r}
p1 = plotQC(ppe@assays@data$Quantification, panel = "pca", cols=grps, labels = colnames(ppe)) +
  ggplot2::ggtitle("Before Batch correction")
p2 = plotQC(ppe@assays@data$normalised, cols=grps, labels = colnames(ppe), panel="pca") +
